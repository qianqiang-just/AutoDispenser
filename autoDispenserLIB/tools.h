...
struct RegParas
{
    ...
    bool    read_vertexgraph;   //读取还是写入变形图


    // output path
    std::string out_gt_file;
    std::string out_each_step_info;
    int         num_sample_nodes;  // r
    ...
    RegParas() // default
    {
        ...
        // dynamic welsch method parameters
        use_Dynamic_nu = true;
        Data_nu = 0.0;   // 初始刚体配准时source point到对应target point的距离中位数d
        Smooth_nu = 40;
        Data_initk = 10;
        Data_endk = 0.5;
        stop = 1e-3;
        ...
        // Sample para
        uni_sample_radio = 5;  //5
        print_each_step_info = false;
        read_vertexgraph = false;
