...
#pragma omp parallel for
    ...
    std::cout << "	复制节点耗时: " << end - start << "毫秒" << std::endl;
    // construct kd Tree
...
{
    mat_U0_.resize(3, n_src_vertex_);  // 从target中搜索每个source点最近的对应点 3*n

    // welsch parameters
    weight_d_.resize(n_src_vertex_);   //align term W_d   n*n
    weight_s_.resize(src_mesh_->n_halfedges());  //reg term W_r  n_edge*n_edge
    ...
    igl::median(init_nus, pars_.Data_nu);   //得到中位数d, v_a^max初始化为10d, 中位数
