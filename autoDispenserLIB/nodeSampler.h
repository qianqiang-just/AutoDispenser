...
//        Scalar sample(Mesh &mesh, Scalar sampleRadiusRatio, sampleAxis axis);
        ...
        //读取文件
        void ReadVertexGraph(Mesh& mesh, std::string path);
    ...
    private:
        ...
        std::vector<std::pair<size_t, size_t>> m_nodeContainer;  // 记录节点index所对应的顶点index
        std::vector<std::map<size_t, Scalar>> m_vertexGraph; // vertex node graph  记录所有顶点受节点的权重影响
        std::vector<std::map<size_t, Scalar>> m_nodeGraph;  //记录每个节点的邻居节点
